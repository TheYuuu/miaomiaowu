...
export default class saleContractModifyClauseEdit extends saleContractPage {
  contractCode: string = '';
  mainContractEffectiveTime: string= '';
  ...
  initCreate() {
    ...
    vm.appApi.getLatestData({
      ...
      if (status) {
        vm.initPageTitle();
        vm.handleData(data);
        vm.basicInfo.supplementaryType = 'MODIFY_CLAUSE';
  ...
  getMainContractTime() {
    ...
    vm.appApi.getPrimaryCode({
      ...
      if (status) {
        vm.mainContractEffectiveTime = data.saleBaseInfoVo.effectiveTime;
        vm.basicInfo.expirationTime = data.saleBaseInfoVo.expirationTime;
      }
    });
  }
  ...
  initPageTitle() {
    ...
    if (vm.method === 'edit') {
      vm.pageTitle = '合同修改-条款修改';
    } else if (from === 'list') {
      vm.pageTitle = '合同修改-条款修改';
    }  else if (from === 'apply') {
      vm.pageTitle = '我的申请 - 条款修改';
    } else {
      // do nithing
    }
  }
    ...
    vm.appApi.saveDraft(ReqData).then(
      ({ data: { status, data} }: any) => {
        if (status) {
          ...
          setTimeout(() => {
            vm.remote.$state.go('app.contract.editSaleContract.modifyClauseEdit', {
              id: data,
              method: 'edit',
              from: 'list'
            });
